...
partial class Util_e
{
    ...
    static void AugmentAlpha()
    {
        ...
        foreach (Object obj in textures)
        {
            ...
            Texture2D newTexture = AugmentAlpha(originalTexture);

            // Encode la nouvelle texture en PNG
            byte[] bytes = newTexture.EncodeToPNG();

            // Sauvegarde la nouvelle texture
            string newPath = Path.Combine(directory, filename + "_strong.png");
        ...
        Debug.Log(nameof(AugmentAlpha) + " done");
    }

    static Texture2D AugmentAlpha(Texture2D texture)
    {
        ...
        for (int i = 0; i < pixels.Length; i++)
        {
            ...
            pixels[i].a = Mathf.Clamp01(1.3f * pixels[i].a);
        }
